...
@Tag(name = "Fórum", description = "Endpoints para gerenciamento de posts e comentários do fórum")
@RestController
...
public class ForumController {
    ...
    @Operation(summary = "Criar post", description = "Cria um novo post no fórum")
    @ApiResponses(value = {
    ...
    @PostMapping("/posts")
    public ResponseEntity<PostResponseDto> criarPost(@Valid @RequestBody PostRequestDto request) {
        try {
            Aluno autor = alunoRepository.findById(request.getAutorId())
                    .orElseThrow(() -> new IllegalArgumentException("Autor não encontrado"));

            Post post = forumService.criarTopico(autor, request.getTitulo(), request.getConteudo());
            return ResponseEntity.status(HttpStatus.CREATED).body(PostResponseDto.fromEntitySimple(post));
    ...
    public ResponseEntity<List<PostResponseDto>> listarPosts(
        ...
        try {
            ...
            if (autorId != null) {
                Aluno autor = alunoRepository.findById(autorId)
                        .orElseThrow(() -> new IllegalArgumentException("Autor não encontrado"));
    ...
    public ResponseEntity<List<PostResponseDto>> listarPostsPopulares() {
        try {
            List<Post> posts = postRepository.findPostsOrderByTotalComentarios();
            List<PostResponseDto> response = posts.stream()
    ...
    public ResponseEntity<PostResponseDto> atualizarPost(
        ...
        try {
            ...
            return ResponseEntity.ok(PostResponseDto.fromEntitySimple(postAtualizado));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
    ...
    @Operation(summary = "Criar comentário", description = "Adiciona um comentário a um post existente")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Comentário criado com sucesso"),
            @ApiResponse(responseCode = "404", description = "Autor ou post não encontrado"),
            @ApiResponse(responseCode = "400", description = "Dados inválidos")
    })
    @PostMapping("/comments")
    public ResponseEntity<CommentResponseDto> criarComentario(@Valid @RequestBody CommentRequestDto request) {
        try {
            Aluno autor = alunoRepository.findById(request.getAutorId())
                    .orElseThrow(() -> new IllegalArgumentException("Autor não encontrado"));

            Comment comment = forumService.comentarTopico(autor, request.getPostId(), request.getConteudo());
        ...
        try {
            ...
            if (postId != null) {
                Post post = postRepository.findById(postId)
                        .orElseThrow(() -> new IllegalArgumentException("Post não encontrado"));
                comments = commentRepository.findByPostOrderByDataCriacaoAsc(post);
            } else if (autorId != null) {
                Aluno autor = alunoRepository.findById(autorId)
                        .orElseThrow(() -> new IllegalArgumentException("Autor não encontrado"));
                comments = commentRepository.findByAutorOrderByDataCriacaoDesc(autor);
            } else {
    ...
    public ResponseEntity<CommentResponseDto> buscarComentarioPorId(@PathVariable Long id) {
        try {
    ...
    public ResponseEntity<CommentResponseDto> atualizarComentario(
        ...
        try {
            ...
            return ResponseEntity.ok(CommentResponseDto.fromEntity(commentAtualizado));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
    ...
    public ResponseEntity<Integer> contarContribuicoes(@PathVariable Long alunoId) {
        try {
